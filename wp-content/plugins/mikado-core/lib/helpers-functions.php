...
if(!function_exists('roam_mikado_add_user_custom_fields')) {
	/**
	 * Function creates custom social fields for users
	 *
	 * return $user_contact
	 */
	function roam_mikado_add_user_custom_fields($user_contact) {
		/**
		 * Function that add custom user fields
		 **/
		$user_contact['position']   = esc_html__('Position', 'mkdf-core');
	...
	add_filter('user_contactmethods', 'roam_mikado_add_user_custom_fields');
}
...
if ( ! function_exists( 'mkdf_core_set_open_graph_meta' ) ) {
	...
	function mkdf_core_set_open_graph_meta() {
		...
		if ( roam_mikado_option_get_value( 'enable_open_graph' ) === 'yes' ) {
			...
			if ( roam_mikado_is_woocommerce_installed() && ( function_exists( 'is_shop' ) && is_shop() ) ) {
			...
			if ( ! empty( $id ) ) {
				...
				if ( get_post_thumbnail_id( $id ) !== '' ) {
					...
					$image = roam_mikado_option_get_value( 'open_graph_image' );
				}
			} else {
				global $wp;
	...
	add_action( 'roam_mikado_header_meta', 'mkdf_core_set_open_graph_meta' );
}