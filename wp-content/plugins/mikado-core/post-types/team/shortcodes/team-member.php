...
class TeamMember implements Lib\ShortcodeInterface {
    ...
    public function __construct() {
        $this->base = 'mkdf_team_member';

        add_action('vc_before_init', array($this, 'vcMap'));

	    //Portfolio project id filter
	    add_filter( 'vc_autocomplete_mkdf_team_member_member_id_callback', array( &$this, 'teamMemberIdAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Portfolio project id render
	    add_filter( 'vc_autocomplete_mkdf_team_member_member_id_render', array( &$this, 'teamMemberIdAutocompleteRender', ), 10, 1 ); // Render exact portfolio. Must return an array (label,value)
    }
    ...
    public function vcMap() {
        if(function_exists('vc_map')) {
	        vc_map( array(
			        'name'                      => esc_html__( 'Mikado Team Member', 'mkdf-core' ),
			        'base'                      => $this->getBase(),
			        ...
			        'params'                    => array(
                        array(
                            ...
                            'heading'     => esc_html__('Team Layout', 'mkdf-core'),
                            'value'       => array(
                        ...
                        array(
					        ...
					        'heading'    => esc_html__( 'Select Team Member', 'mkdf-core' ),
					        'settings'   => array(
						        'sortable'      => true,
						        'unique_values' => true
					        ),
					        'description' => esc_html__( 'If you left this field empty then project ID will be of the current page', 'mkdf-core' )
				        )
			        )
		        )
	        );
    ...
    public function render($atts, $content = null) {
        ...
        $html = mkdf_core_get_cpt_shortcode_module_template_part('team', 'team-template-'.$params['team_member_layout'], '', $params);
