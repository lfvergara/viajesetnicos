...
class Separator implements Lib\ShortcodeInterface {
	...
	private function getHolderStyles( $params ) {
		...
		if ( $params['width'] !== '' ) {
			if ( roam_mikado_string_ends_with( $params['width'], '%' ) || roam_mikado_string_ends_with( $params['width'], 'px' ) ) {
				$styles[] = 'width: ' . $params['width'];
			} else {
				$styles[] = 'width: ' . roam_mikado_filter_px( $params['width'] ) . 'px';
			}
		}
		
		if ( $params['thickness'] !== '' ) {
			$styles[] = 'border-bottom-width: ' . roam_mikado_filter_px( $params['thickness'] ) . 'px';
		}
		
		if ( $params['top_margin'] !== '' ) {
			if ( roam_mikado_string_ends_with( $params['top_margin'], '%' ) || roam_mikado_string_ends_with( $params['top_margin'], 'px' ) ) {
				$styles[] = 'margin-top: ' . $params['top_margin'];
			} else {
				$styles[] = 'margin-top: ' . roam_mikado_filter_px( $params['top_margin'] ) . 'px';
			}
		}
		
		if ( $params['bottom_margin'] !== '' ) {
			if ( roam_mikado_string_ends_with( $params['bottom_margin'], '%' ) || roam_mikado_string_ends_with( $params['bottom_margin'], 'px' ) ) {
				$styles[] = 'margin-bottom: ' . $params['bottom_margin'];
			} else {
				$styles[] = 'margin-bottom: ' . roam_mikado_filter_px( $params['bottom_margin'] ) . 'px';
			}
		}
