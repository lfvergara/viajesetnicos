...
class Icon implements Lib\ShortcodeInterface {
		    ...
		    vc_map(
		    	array(
				    ...
				    'params'                    => array_merge(
					    ...
					    \RoamMikadoIconCollections::get_instance()->getVCParamsArray(),
					    array(
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'size',
                            ...
                            array(
                                'type'        => 'dropdown',
                                'heading'     => esc_html__('Hover Type', 'mkdf-core'),
                                'param_name'  => 'hover_type',
                                'value'       => array(
                                    esc_html__('Default', 'mkdf-core') => 'hover_default',
                                    esc_html__('Classic', 'mkdf-core') => 'hover_classic',
                                ),
                                'dependency' => array(
                                    'element' => 'type',
	...
	public function render( $atts, $content = null ) {
		...
		$default_atts = array_merge( $default_atts, roam_mikado_icon_collections()->getShortcodeParams() );
		$params       = shortcode_atts( $default_atts, $atts );
		
		$iconPackName = roam_mikado_icon_collections()->getIconCollectionParamNameByKey( $params['icon_pack'] );

        if($params['type'] == 'mkdf-normal') {
            $params['hover_type'] = 'hover_classic';
        }
        ...
        $params['icon_holder_hover_styles']		= $this->generateIconHolderHoverStyles($params);
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'mkdf-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'mkdf-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . roam_mikado_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'mkdf-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . roam_mikado_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . roam_mikado_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . roam_mikado_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . roam_mikado_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'mkdf-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . roam_mikado_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
    ...
    /**
     * Generates icon holder styles for circle and square icon type when default hover type is used
     *
     * @param $params
     *
     * @return array
     */
    private function generateIconHolderHoverStyles($params) {
        $iconHolderHoverStyles = array();

        //generate styles attribute only if type isn't normal
        if(isset($params['type']) && $params['type'] !== 'mkdf-normal') {
            if(!empty($params['hover_background_color'])) {
                $iconHolderHoverStyles[] = 'background-color: '.$params['hover_background_color'];
            }
            if(!empty($params['hover_border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
                $iconHolderHoverStyles[] = 'border-style: solid';
                $iconHolderHoverStyles[] = 'border-color: '.$params['hover_border_color'];
                $iconHolderHoverStyles[] = 'border-width: '.roam_mikado_filter_px($params['border_width']).'px';
                $iconHolderHoverStyles[] = 'top: -'.$params['border_width'].'px';
                $iconHolderHoverStyles[] = 'left: -'.$params['border_width'].'px';
            } else if(isset($params['border_width']) && $params['border_width'] !== ''){
                $iconHolderHoverStyles[] = 'border-style: solid';
                $iconHolderHoverStyles[] = 'border-width: '.roam_mikado_filter_px($params['border_width']).'px';
                $iconHolderHoverStyles[] = 'top: -'.$params['border_width'].'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderHoverStyles[] = 'border-radius: '.roam_mikado_filter_px($params['border_radius']).'px';
                }
            }
        }

        return $iconHolderHoverStyles;
    }
	
	private function generateIconHolderData( $params ) {
		$iconHolderData = array();
		
		if ( isset( $params['type'] ) && $params['type'] !== 'mkdf-normal' && $params['hover_type'] == 'hover_classic') {
