...
class VideoButton implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
                        ...
                        array(
                            ...
                            'heading'     => esc_html__( 'Play Button Label', 'mkdf-core' ),
                            'description' => esc_html__( 'Label that will be displayed under play button', 'mkdf-core' )
                        ),
					)
				)
			);
	...
	private function getPlayButtonStyles( $params ) {
		...
		if ( ! empty( $params['play_button_size'] ) ) {
			$styles[] = 'font-size: ' . roam_mikado_filter_px( $params['play_button_size'] ) . 'px';
		}
