...
if ( ! function_exists( 'roam_mikado_get_social_share_html' ) ) {
	/**
	 ...
	 * @return mixed|string
	 */
	function roam_mikado_get_social_share_html( $params = array() ) {
		if ( roam_mikado_core_plugin_installed() ) {
			return roam_mikado_execute_shortcode( 'mkdf_social_share', $params );
		}
	}
}

if ( ! function_exists( 'roam_mikado_the_excerpt_max_charlength' ) ) {
	/**
	 ...
	 */
	function roam_mikado_the_excerpt_max_charlength( $charlength ) {
		$twitter_via_meta = roam_mikado_options()->getOptionValue( 'twitter_via' );
