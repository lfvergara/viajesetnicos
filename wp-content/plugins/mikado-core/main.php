...
if(!function_exists('mkd_core_activation')) {
     ...
     * and defines roam_mikado_core_on_activate action
     */
    function mkd_core_activation() {
        do_action('roam_mikado_core_on_activate');
...
if(!function_exists('mkd_core_theme_menu')) {
    ...
    function mkd_core_theme_menu() {
        if (mkdf_core_theme_installed()) {
            ...
            roam_mikado_init_theme_options();
            ...
            $page_hook_suffix = add_menu_page(
                ...
                'roam_mikado_theme_menu',                // The ID used to bind submenu items to this menu
                array($roam_mikado_Framework->getSkin(), 'renderOptions'), // The callback function used to render this menu
                $roam_mikado_Framework->getSkin()->getSkinURI().'/assets/img/admin-logo-icon.png',             // Icon For menu Item
                100           // Position
            );

            foreach ($roam_mikado_Framework->mkdOptions->adminPages as $key=>$value ) {
                $slug = "";
                ...
                $subpage_hook_suffix = add_submenu_page(
                    'roam_mikado_theme_menu',
	                'Mikado Options - '.$value->title,                   // The value used to populate the browser's title bar when the menu page is active
                    $value->title,                   // The text of the menu in the administrator's sidebar
                    'administrator',                  // What roles are able to access the menu
                    'roam_mikado_theme_menu'.$slug,                // The ID used to bind submenu items to this menu
                    array($roam_mikado_Framework->getSkin(), 'renderOptions')
                );

                add_action('admin_print_scripts-'.$subpage_hook_suffix, 'roam_mikado_enqueue_admin_scripts');
                add_action('admin_print_styles-'.$subpage_hook_suffix, 'roam_mikado_enqueue_admin_styles');
            };

            add_action('admin_print_scripts-'.$page_hook_suffix, 'roam_mikado_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$page_hook_suffix, 'roam_mikado_enqueue_admin_styles');
        }
    }
	...
	function mkd_core_theme_menu_backup_options() {
		if (mkdf_core_theme_installed()) {
			...
			$page_hook_suffix = add_submenu_page(
				...
				'administrator',                  // What roles are able to access the menu
				'roam_mikado_theme_menu'.$slug,                // The ID used to bind submenu items to this menu
				array($roam_mikado_Framework->getSkin(), 'renderBackupOptions')
			);
