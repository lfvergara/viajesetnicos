...
class MikadoCoreImport {
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        $result = update_option( 'mkdf_options_roam', $options);
	...
	public function rev_sliders() {
		$rev_sldiers = array(
			...
			'home-1-slider-2.zip',
			'holiday-home.zip',
			'holiday-home-slider-2.zip',
			'main-home.zip',
			'adventure-home.zip',
			'mountaineering-home.zip',
			'camping-home.zip',
			'destination-showcase.zip',
			'landing.zip',
			'landing-2.zip',
			'landing-3.zip'
		);
    ...
    function mkdf_admin_import() {
        if (mkdf_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                array($roam_mikado_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'roam_mikado_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'roam_mikado_enqueue_admin_styles');
        }
    }
}