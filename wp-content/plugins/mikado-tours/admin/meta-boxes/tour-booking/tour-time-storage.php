...
class TourTimeStorage {
	...
	public function handleAjaxSaving() {
		...
		if(!$this->isAuthorizedRequest(self::AJAX_REQUEST)) {
			...
			$returnObject->message = esc_html__('Un-authorized request', 'mkdf-tours');
		...
		if(!$returnObject->status) {
			$returnObject->message = esc_html__('There was some error saving tour periods. Please try again later.', 'mkdf-tours');
		} else {
			$returnObject->message = esc_html__('You have successfully saved tour periods.', 'mkdf-tours');
		}
