...
class BookingHandler {
	...
	public function handleBooking() {
		...
		if(!$returnObject->status) {
			...
			if(mkdf_tours_theme_installed()) {
				$redirectOptionValue = roam_mikado_options()->getOptionValue('tours_checkout_page');
	...
	private function setLabels() {
		$this->labels = array(
			'name'             => esc_html__('Por favor escribe tu nombre', 'mkdf-tours'),
			'email'            => esc_html__('Por favor escribe un email válido', 'mkdf-tours'),
			'confirmEmail'     => esc_html__('El correo electrónico y el campo de correo electrónico de confirmación no son el mismo', 'mkdf-tours'),
			'date'             => esc_html__('Por favor elije una fecha', 'mkdf-tours'),
			'dateNotAvailable' => esc_html__('La fecha elegida no está disponible', 'mkdf-tours'),
			'time'             => esc_html__('Por favor elige el horario', 'mkdf-tours'),
			'numberOfTickets'  => esc_html__('Por favor, elija el número de personas', 'mkdf-tours')
		);
	}
	...
	private function sendUserConfirmationEmail($id, $bookingEmail, $bookingHash) {
		...
		$message = esc_html__('Hemos recibido su petición de información para','mkdf-tours') .  ' '. $tour_title . ' '. esc_html__('El equipo de viajes étnicos le respondera a la mayor brevedad.', 'mkdf-tours');

		$subject  = esc_html__('Información de reserva para', 'mkdf-tours') . ' '. $tour_title . ' '. esc_html__('de', 'mkdf-tours') . ' ' . $name;
	...
	private function sendAdminConfirmationEmail($id, $bookingEmail, $bookingHash) {
		...
		if(roam_mikado_options()->getOptionValue('enable_admin_booking_email') == 'yes'){
			//Get email address
			if(roam_mikado_options() -> getOptionValue('admin_email') != '') {
				$mail_to = roam_mikado_options() -> getOptionValue('admin_email');
			} else {
			...
			$message = esc_html__('Ha recibido una solicitud de reserva para','mkdf-tours') .  ' '. $tour_title . ' .';
			...
			$messageTemplate = esc_html__('Atención', 'mkdf-tours') . ': ' . $message . "\r\n\n";
			$messageTemplate .= esc_html__('De', 'mkdf-tours'). ': ' . $booking->user_name . "\r\n";
			$messageTemplate .= esc_html__('Correo electrónico', 'mkdf-tours'). ': ' . $booking->user_email . "\r\n";
			$messageTemplate .= esc_html__('Mensaje', 'mkdf-tours') . ': ' . $booking->user_message . "\r\n";
