...
class TourSearch {
	...
	private function setPropertiesFromRequest() {
		...
		if(mkdf_tours_theme_installed()) {
			$defaultViewType = roam_mikado_options()->getOptionValue('tours_search_default_view_type');

			$orderingOption = roam_mikado_options()->getOptionValue('tours_search_default_ordering');
		...
		if(mkdf_tours_theme_installed()) {
			$this->toursPerPage = roam_mikado_options()->getOptionValue('tours_per_page');
